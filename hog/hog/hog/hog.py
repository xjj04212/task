...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        num_rolls = free_bacon(opponent_score)
    else:
        num_rolls = roll_dice(num_rolls,dice)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1,11):
        averaged = make_averaged(rollDice,trials_count)
        current = averaged()
        if current > max:
            max = current
            rolls = i
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    elif score + 5 >= 100 or extra_turn(score + 5, opponent_score) :
        return 2
    
    elif score + 7 >= 100 or extra_turn(score + 7, opponent_score):
        return 3
